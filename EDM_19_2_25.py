...
address_id_counter = address_id_counter_2 = primary_id_counter_2 = loc_id_counter = loc_id_counter_2 = loc_id_counter_3 = loc_id_counter_4 = primary_id_counter = loccvg_id_counter = eqdet_counter = hudet_counter = todet_counter = fldet_counter =loc_id_hu= int(LOCID_1)
loc_id_hu_2= int(LOCID_1)
        ...
        for row in chunks[i].iter_rows(named=True):
            ...
            for table_col in all_columns:
                if table_col == 'HUDETID':
                    mapped_columns_hu.append(table_col)
                    mapped_values_hu.append(f"{hudet_counter}")
                elif table_col == 'LOCID':
                    mapped_columns_hu.append(table_col)
                    mapped_values_hu.append(f"{loc_id_hu_2}")
                elif table_col in table_mappings["hudet"]:
                    df_col = table_mappings["hudet"][table_col]
                    if df_col in row:
                        mapped_columns_hu.append(table_col)
                        mapped_values_hu.append(f"'{row[df_col]}'")
                elif table_col not in foreign_key_columns:
                    default_behavior = unspecified_column_behavior["hudet"]
                    if table_col in default_behavior["null_columns"]:
                        mapped_columns_hu.append(table_col)
                        mapped_values_hu.append("NULL")
                    elif table_col in default_behavior["blank_columns"]:
                        mapped_columns_hu.append(table_col)
                        mapped_values_hu.append("' '")
                    elif table_col in default_behavior["zero_columns"]:
                        mapped_columns_hu.append(table_col)
                        mapped_values_hu.append("0")
                    elif table_col in default_behavior["specific_defaults"]:
                        mapped_columns_hu.append(table_col)
                        mapped_values_hu.append(f"'{default_behavior['specific_defaults'][table_col]}'")
                    else:
                        mapped_columns_hu.append(table_col)
                        mapped_values_hu.append(default_behavior["default"])

            # Ensure the number of columns matches the number of values
            if len(mapped_columns_hu) == len(mapped_values_hu):
                sql_command = f"INSERT INTO hudet ({', '.join(mapped_columns_hu)}) VALUES ({', '.join(mapped_values_hu)})"
                try:
                    sql_conn.execute(sql_command)
                    # Increment the HUDETID and LOCID counters
                    if 'HUDETID' in mapped_columns_hu:
                        hudet_counter += 1
                    if 'LOCID' in mapped_columns_hu:
                        loc_id_hu_2 = loc_id_hu_2 + 1
                except Exception as e:
                    print(f"Error executing command: {sql_command}")
                    print(f"Exception: {e}")
            else:
            ...
            for table_name, column_mapping in table_mappings.items():
                ...
                for table_col in all_columns:
                    if table_col == 'TODETID':
                        ...
                        mapped_columns.append(table_col)
                        mapped_values.append(f"{loc_id_counter_4}")
            ...
            for table_name, column_mapping in table_mappings.items():
                ...
                for table_col in all_columns:
                    if table_col == 'FLDETID':
                        ...
                        mapped_columns.append(table_col)
                        mapped_values.append(f"{loc_id_counter_3}")
                    elif table_col in column_mapping:
                        df_col = column_mapping[table_col]
                        if df_col in row:
                            mapped_columns.append(table_col)
